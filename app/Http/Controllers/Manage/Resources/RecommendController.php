...
class RecommendController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $list = Res_Recommend::where(function ($query) use ($request) {
                if ($request->key) {
            ...
            return view('manage.resources.recommend.index', compact('list'));
        } catch (Exception $ex) {
    ...
    {
        return view('manage.resources.recommend.create');
    }
    ...
    {
        try {
            $item = Res_Recommend::find($request->id);
            if (!$item) {
                throw new Exception("数据错误,未找到团期推荐标签！");
            }
            return view('manage.resources.recommend.edit', compact('item'));
