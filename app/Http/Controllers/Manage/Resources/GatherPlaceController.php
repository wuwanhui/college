...
//常用集合地管理
//*/
class GatherPlaceController extends BaseController
{
    /**
     * 常用集合地列表
     */
    public function index(Request $request)
    {
        ...
        try {
            $list = Res_GatherPlace::where(function ($query) use ($request) {
                if ($request->key) {
            ...
            return view('manage.resources.gatherplace.index', compact('list'));
        } catch (Exception $ex) {
    ...
    {
        return view('manage.resources.gatherplace.create');
    }
    ...
    {
        try {
            $item = Res_GatherPlace::find($request->id);
            if (!$item) {
                throw new Exception("数据错误,未找到常用集合地信息！");
            }
            return view('manage.resources.gatherplace.edit', compact('item'));
