...
class ScenicspotController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $list = Res_Scenicspot::where(function ($query) use ($request) {
                if ($request->key) {
            ...
            return view('manage.resources.scenicspot.index', compact('list'));
        } catch (Exception $ex) {
    ...
    {
        return view('manage.resources.scenicspot.create');
    }
    ...
    {
        try {
            $item = Res_Scenicspot::find($request->id);
            if (!$item) {
                throw new Exception("数据错误,未找到常用景点信息！");
            }
            return view('manage.resources.scenicspot.edit', compact('item'));
