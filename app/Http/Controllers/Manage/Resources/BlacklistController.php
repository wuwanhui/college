...
class BlacklistController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $list = Res_BlackList::where(function ($query) use ($request) {
                if ($request->key) {
            ...
            return view('manage.resources.blacklist.index', compact('list'));
        } catch (Exception $ex) {
    ...
    {
        return view('manage.resources.blacklist.create');
    }
    ...
    {
        try {
            $item = Res_BlackList::find($request->id);
            if (!$item) {
                throw new Exception("数据错误,未找到黑名单信息！");
            }
            return view('manage.resources.blacklist.edit', compact('item'));
