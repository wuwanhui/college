...
//常用公司抬头管理
//*/
class LetterheadController extends BaseController
{
    /**
     * 列表
     */
    public function index(Request $request)
    {
        ...
        try {
            $list = Res_Letterhead::where(function ($query) use ($request) {
                if ($request->key) {
                    $query->where('name', 'like', '%' . $request->key . '%'); //按名称模糊查询
                }
            ...
            return view('manage.resources.letterhead.index', compact('list'));
        } catch (Exception $ex) {
    ...
    {
        return view('manage.resources.letterhead.create');
    }
    ...
    {
        try {
            $item = Res_Letterhead::find($request->id);
            if (!$item) {
                throw new Exception("数据错误,未找到公司抬头信息！");
            }
            return view('manage.resources.letterhead.edit', compact('item'));
