...
class GuideController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $list = Res_Guide::where(function ($query) use ($request) {
                if ($request->key) {
                    $query->where('name', 'like', '%' . $request->key . '%'); //按名称或单位代码模糊查询
                }
            ...
            return view('manage.resources.guide.index', compact('list'));
        } catch (Exception $ex) {
    ...
    {
        return view('manage.resources.guide.create');
    }
    ...
    {
        try {
            $item = Res_Guide::find($request->id);
            if (!$item) {
                throw new Exception("数据错误,未找到导游领队信息！");
            }
            return view('manage.resources.guide.edit', compact('item'));
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            if (!isset($request->id)) {
                ...
                $typeArray = $request->language;
                $item->language = implode(',', $typeArray);
            } else {
                //编辑保存
                $item = Res_Guide::find($request->id);
                $item->fill($request->all()); //将表单元素内容填充到对象
