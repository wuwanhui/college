                            ...
                            <table class="table table-bordered table-hover  table-condensed">
                                ...
                                <tr v-for="item in list.data">
                                    ...
                                    <td style="text-align: center">
                                        ...
                                        |
                                        <a href="{{url('/manage/weixin/qrcode/follow')}}?id=@{{ item.id }}">发送</a>
                                        |
                                        <a href="{{url('/manage/weixin/qrcode/follow')}}?id=@{{ item.id }}">绑定</a>
...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'weixin', item: 'qrcode'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载数据
                    this.$http.get("{{url('/manage/weixin/qrcode?json')}}", {params: this.params})
                ...
                create: function () {
                    window.location.href = '{{url('/manage/weixin/qrcode/create')}}';
                },
                edit: function (item) {
                    this.customer = item;
                    window.location.href = '{{url('/manage/weixin/qrcode/edit')}}' + '?id=' + item.id;
                },
                detail: function (item) {
                    this.customer = item;
                    window.location.href = '{{url('/manage/weixin/qrcode/detail')}}' + '?id=' + item.id;
                },
                update: function (item) {
                    var _self = this;
                    //加载数据
                    this.$http.post("{{url('/manage/weixin/qrcode/update')}}", item)
                            .then(function (response) {
                                        if (response.data.code == 0) {
                                            msg('更新成功！');
                                            _self.init();
                ...
                sync: function () {
                    var _self = this;
                    //加载数据
                    this.$http.post("{{url('/manage/weixin/qrcode/sync')}}")
                            .then(function (response) {
                                        if (response.data.code == 0) {
                                            _self.list = response.data.data;
                                            return
                ...
                remark: function (item) {
                    ...
                    layer.prompt({title: '输入任何备注，最多30个字符', formType: 2}, function (remark, index) {
                        ...
                        layer.close(index);
                        //加载数据
                        _self.$http.post("{{url('/manage/weixin/qrcode/remark')}}", item)
                                .then(function (response) {
                                            if (response.data.code == 0) {
                                                msg('备注成功');
                                                _self.init();
                ...
                state: function (item) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '{{url('/manage/weixin/qrcode/edit')}}',
                        data: _self.role,
                        headers: {
                ...
                delete: function (ids) {
                    ...
                    layer.confirm('确认删除吗？', {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '{{url('/manage/weixin/qrcode/delete')}}',
                            data: {ids: ids},
                            headers: {
