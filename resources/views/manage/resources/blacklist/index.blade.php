                ...
                <fieldset>
                    <table class="table table-bordered table-hover  table-condensed">
                        ...
                        <tr v-for="item in list.data">
                            ...
                            <td style="text-align: center" v-text="item.certType"></td>
                            <td style="text-align: center" v-text="item.certNo"></td>
...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'resources', item: 'blacklist'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载列表
                    this.$http.get("{{url('/manage/resources/blacklist?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/resources/blacklist/create')}}', '新增黑名单', 600, 600);
                },
                edit: function (item) {
                    //编辑
                    openUrl('{{url('/manage/resources/blacklist/edit')}}?id=' + item.id, '编辑黑名单', 600, 600);
                },
                delete: function (ids) {
                    if (ids == "") {
                    ...
                    var _self = this;
                    //删除方法
                    layer.confirm('您确认要删除选择的黑名单？', {
                        btn: ['确认', '取消']
                    }, function () {
                        _self.$http.post("{{url('/manage/resources/blacklist/delete')}}", {ids: ids}).then(function (resspose) {
