                ...
                <fieldset>
                    <table class="table table-bordered table-hover  table-condensed">
                        ...
                        <tr v-for="item in list.data">
                            ...
                            <td style="text-align: center"><span class="label" style="background-color: @{{ item.colour }}">@{{ item.name }}</span></td>
                            <td style="text-align: center" v-text="item.colour"></td>
...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'resources', item: 'recommend'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载列表
                    this.$http.get("{{url('/manage/resources/recommend?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/resources/recommend/create')}}', '新增推荐标签', 600, 600);
                },
                edit: function (item) {
                    //编辑
                    openUrl('{{url('/manage/resources/recommend/edit')}}?id=' + item.id, '编辑推荐标签', 600, 600);
                },
                delete: function (ids) {
                    if (ids == "") {
                        layer.msg('请选择要删除的标签！');
                    ...
                    var _self = this;
                    //删除方法
                    layer.confirm('您确认要删除选择的标签？', {
                        btn: ['确认', '取消']
                    }, function () {
                        _self.$http.post("{{url('/manage/resources/recommend/delete')}}", {ids: ids}).then(function (resspose) {
