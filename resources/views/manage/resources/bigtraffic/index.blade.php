                ...
                <fieldset>
                    <table class="table table-bordered table-hover  table-condensed">
                        ...
                        <tr v-for="item in list.data">
                            ...
                            <td style="text-align: center" v-text="item.trend_cn"></td>
                            <td style="text-align: center" v-text="item.type_cn"></td>
                            <td style="text-align: center">@{{ item.departurePlace }}-@{{ item.destination }}</td>
                            <td style="text-align: center" v-text="item.flightNumer"></td>
                            <td style="text-align: center">@{{ item.hour }}:@{{ item.minute }}</td>
                            <td style="text-align: center" v-text="item.company"></td>
...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'resources', item: 'gatherplace'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                ...
                create: function () {
                    openUrl('{{url('/manage/resources/bigtraffic/create')}}', '新增大交通', 600, 600);
                },
                edit: function (item) {
                    //编辑
                    openUrl('{{url('/manage/resources/bigtraffic/edit')}}?id=' + item.id, '编辑大交通', 600, 600);
                },
                delete: function (ids) {
                    if (ids == "") {
                        layer.msg('请选择要删除的大交通！');
                        return;
                    }

                    var _self = this;
                    //删除方法
                    layer.confirm('您确认要删除选择的大交通？', {
                        btn: ['确认', '取消']
                    }, function () {
                        _self.$http.post("{{url('/manage/resources/bigtraffic/delete')}}", {ids: ids}).then(function (resspose) {
