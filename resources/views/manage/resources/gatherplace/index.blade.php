                ...
                <fieldset>
                    <table class="table table-bordered table-hover  table-condensed">
                        ...
                        <tr v-for="item in list.data">
                            ...
                            <td style="text-align: center" v-text="item.departureCity"></td>
        ...
        var vm = new Vue({
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载列表
                    this.$http.get("{{url('/manage/resources/gatherplace?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/resources/gatherplace/create')}}', '新增集合地', 600, 600);
                },
                edit: function (item) {
                    //编辑
                    openUrl('{{url('/manage/resources/gatherplace/edit')}}?id=' + item.id, '编辑集合地', 600, 600);
                },
                delete: function (ids) {
                    if (ids == "") {
                        layer.msg('请选择要删除的集合地！');
                        return;
                    }

                    var _self = this;
                    //删除方法
                    layer.confirm('您确认要删除选择的集合地？', {
                        btn: ['确认', '取消']
                    }, function () {
                        _self.$http.post("{{url('/manage/resources/gatherplace/delete')}}", {ids: ids}).then(function (resspose) {
