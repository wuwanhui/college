                ...
                <fieldset>
                    <table class="table table-bordered table-hover  table-condensed">
                        ...
                        <tr v-for="item in list.data">
                            ...
                            <td v-text="item.mobilePhone"></td>
                            <td>@{{item.deptName }} @{{  item.position }}</td>
                            <td v-text="item.telePhone"></td>
                            <td v-text="item.qq"></td>
                            <td v-text="item.fax"></td>
        ...
        var vm = new Vue({
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载供应商数据
                    this.$http.get("{{url('/manage/resources/supplier/contacts?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/resources/supplier/contacts/create')}}', '新增联系人', 800, 500);
                },
                edit: function (item) {
                    //编辑联系人
                    openUrl('{{url('/manage/resources/supplier/contacts/edit')}}?id=' + item.id, '编辑联系人', 800, 500);
                },
                delete: function (ids) {
                    if (ids == "") {
                        layer.msg('请选择要删除的联系人！');
                        return;
                    }

                    var _self = this;
                    //删除方法
                    layer.confirm('您确认要删除选择的联系人？', {
                        btn: ['确认', '取消']
                    }, function () {
                        //提交修改保存
                        _self.$http.post("{{url('/manage/resources/supplier/contacts/delete')}}", {ids: ids}).then(function (resspose) {
                ...
                search: function (bg) {
                    if(bg){this.params={page: 1, state: -1,id:'{{$supplier->id}}'};}
                    //搜索方法
                    this.init();
                }
            }
