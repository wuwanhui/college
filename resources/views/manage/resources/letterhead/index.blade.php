...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'resources', item: 'letterhead'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载列表
                    this.$http.get("{{url('/manage/resources/letterhead?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/resources/letterhead/create')}}', '新增公司抬头', 600, 600);
                },
                edit: function (item) {
                    //编辑
                    openUrl('{{url('/manage/resources/letterhead/edit')}}?id=' + item.id, '编辑公司抬头', 600, 600);
                },
                delete: function (ids) {
                    if (ids == "") {
                        layer.msg('请选择要删除的公司抬头！');
                        return;
                    }

                    var _self = this;
                    //删除方法
                    layer.confirm('您确认要删除选择的公司抬头？', {
                        btn: ['确认', '取消']
                    }, function () {
                        _self.$http.post("{{url('/manage/resources/letterhead/delete')}}", {ids: ids}).then(function (resspose) {
