...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'resources', item: 'scenicspot'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载供应商数据
                    this.$http.get("{{url('/manage/resources/scenicspot?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/resources/scenicspot/create')}}', '新增景点', 800, 500);
                },
                edit: function (item) {
                    //编辑联系人
                    openUrl('{{url('/manage/resources/scenicspot/edit')}}?id=' + item.id, '编辑景点', 800, 500);
                },
                delete: function (ids) {
                    if (ids == "") {
                        layer.msg('请选择要删除的景点！');
                    ...
                    var _self = this;
                    //删除方法
                    layer.confirm('您确认要删除选择景点？', {
                        btn: ['确认', '取消']
                    }, function () {
                        //提交修改保存
                        _self.$http.post("{{url('/manage/resources/scenicspot/delete')}}", {ids: ids}).then(function (resspose) {
