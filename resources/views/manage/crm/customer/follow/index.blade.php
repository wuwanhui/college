                        ...
                        <form method="Post" class="form-inline">
                            <table class="table table-bordered table-hover  table-condensed">
                                ...
                                <tr style="text-align: center" class="text-center">
                                    ...
                                    <th><a href="">所属客户</a></th>
                                    <th><a href="">标题</a></th>
                                    <th style="width: 100px;"><a href="">处理状态</a></th>
                                    <th><a href="">内部备注</a></th>
                                ...
                                <tr v-for="item in list.data">
                                    ...
                                    <td>
                                        <span v-text="item.customer.name"></span>
                                        <hr>
                                        <span v-text="item.linkman.name"></span><span v-text="item.type_cn"></span>
...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'crm', item: 'customerFollow'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载数据
                    this.$http.get("{{url('/manage/crm/customer/follow?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/crm/customer/follow/create')}}', '新增联系记录', 800, 600);
                },
                edit: function (item) {
                    this.follow = item;
                    openUrl('{{url('/manage/crm/customer/follow/edit')}}' + '?id=' + item.id, '编辑"' + item.name + '"联系记录', 800, 600);
                },
                state: function (item) {
                    var _self = this;
                    this.follow = item;
                    this.follow.state = this.follow.state == 0 ? 1 : 0;

                    $.ajax({
                        type: 'POST',
                        url: '{{url('/manage/crm/customer/follow/edit')}}',
                        data: _self.follow,
                        headers: {
                ...
                delete: function (ids) {
                    ...
                    layer.confirm('确认删除吗？', {
                        ...
                        _self.$http.post("{{url('/manage/crm/customer/follow/delete')}}", {ids: ids})
                                .then(function (response) {
                                            if (response.data.code == 0) {
                                                _self.init();
                                                layer.closeAll();
