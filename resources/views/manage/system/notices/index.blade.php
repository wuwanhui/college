...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'system', item: 'notices'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载数据
                    this.$http.get("{{url('/manage/system/notices?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/system/notices/create')}}', '新增公告', 800, 600);
                },
                edit: function (item) {
                    this.notices = item;
                    openUrl('{{url('/manage/system/notices/edit')}}', '编辑"' + item.name + '"公告', 600, 300);
                },
                state: function (item) {
                    var _self = this;
                    this.notices = item;
                    this.notices.state = this.notices.state == 0 ? 1 : 0;

                    $.ajax({
                        type: 'POST',
                        url: '{{url('/manage/system/notices/edit')}}',
                        data: _self.notices,
                        headers: {
                ...
                delete: function (ids) {
                    ...
                    layer.confirm('确认删除吗？', {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '{{url('/manage/system/notices/delete')}}',
                            data: {ids: ids},
                            headers: {
