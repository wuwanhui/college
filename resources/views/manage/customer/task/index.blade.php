                        ...
                        <form method="Post" class="form-inline">
                            <table class="table table-bordered table-hover  table-condensed">
                                ...
                                <tr style="text-align: center" class="text-center">
                                    ...
                                    <th><a href="">任务标题</a></th>
                                    <th><a href="">内容</a></th>
                                    <th style="width: 60px;"><a href="">类别</a></th>
                                    <th style="width: 100px;"><a href="">积分</a></th>
                                ...
                                <tr v-for="item in list.data">
                                    ...
                                    <td style="text-align: center" v-text="item.endTime"></td>
...
@section('script')
    <script type="application/javascript">
        //sidebar.menu = {type: 'crm', item: 'customerTask'};
        var vm = new Vue({
            el: '.content',
            data: {
            ...
            methods: {
                init: function () {
                    var _self = this;
                    //加载数据
                    this.$http.get("{{url('/manage/crm/customer/task?json')}}", {params: this.params})
                ...
                create: function () {
                    openUrl('{{url('/manage/crm/customer/task/create')}}', '新增任务', 800, 400);
                },
                edit: function (item) {
                    this.task = item;
                    openUrl('{{url('/manage/crm/customer/task/edit')}}' + '?id=' + item.id, '编辑"' + item.name + '"任务', 800, 400);
                },
                state: function (item) {
                    var _self = this;
                    this.task = item;
                    this.task.state = this.task.state == 0 ? 1 : 0;

                    $.ajax({
                        type: 'POST',
                        url: '{{url('/manage/crm/customer/task/edit')}}',
                        data: _self.task,
                        headers: {
                ...
                delete: function (ids) {
                    ...
                    layer.confirm('确认删除吗？', {
                        ...
                        _self.$http.post("{{url('/manage/crm/customer/task/delete')}}", {ids: ids})
                                .then(function (response) {
                                            if (response.data.code == 0) {
                                                _self.init();
                                                layer.closeAll();
